...
static int ElfW(is_elf, )(struct pax_state * const state) {
  ...
  if (ehdr->e_ehsize != sizeof(ElfW(Elf, _Ehdr))) { return 0; }
  if ((ehdr->e_version != EV_CURRENT) || (ehdr->e_ident[EI_CLASS] != ElfW(ELFCLASS, ))) { return 0; }
  if ((ehdr->e_type != ET_EXEC) && (ehdr->e_type != ET_DYN)) { return 0; }
  if (!ehdr->e_phoff || !ehdr->e_phnum || sizeof(ElfW(Elf, _Phdr)) != ehdr->e_phentsize) { return 0; }
  if (ehdr->e_phnum > 65536U / ehdr->e_phentsize - 1) { return 0; }
  if (ehdr->e_phoff > ehdr->e_phoff + (ElfW(Elf, _Off))ehdr->e_phentsize * ehdr->e_phnum) { return 0; }
  if (state->size < ehdr->e_phoff + (ElfW(Elf, _Off))ehdr->e_phentsize * ehdr->e_phnum) { return 0; }
  if (ehdr->e_shoff) {
    if (!ehdr->e_shnum || sizeof(ElfW(Elf, _Shdr)) != ehdr->e_shentsize) { return 0; }
    if (ehdr->e_shnum > 65536U / ehdr->e_shentsize) { return 0; }
    if (ehdr->e_shoff > ehdr->e_shoff + (ElfW(Elf, _Off))ehdr->e_shentsize * ehdr->e_shnum) { return 0; }
    if ((Elf32_Off)state->size < ehdr->e_shoff + (ElfW(Elf, _Off))ehdr->e_shentsize * ehdr->e_shnum) { return 0; }
  }
